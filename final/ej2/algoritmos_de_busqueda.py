...
# Las modificaciones al código y la traducción han realizadas por José Luis
# Ruiz Reina (Depto. de Ciencias de la Computación e Inteligencia Artificial
...
def búsqueda_generica(problema, abiertos):
...
def búsqueda_en_profundidad(problema):
    return búsqueda_generica(problema, PilaLIFO())


def búsqueda_en_anchura(problema):
    return búsqueda_generica(problema, ColaFIFO())
...
def búsqueda_con_prioridad(problema, f):
...
def búsqueda_coste_uniforme(problema):
    """Búsqueda óptima: gestiona la cola en orden creciente de coste del camino"""
    return búsqueda_con_prioridad(problema, lambda x: x.coste_camino)


def búsqueda_primero_el_mejor(problema, h=None):
    ...
    return búsqueda_con_prioridad(problema,lambda n: h(n.estado))


def búsqueda_a_estrella(problema, h=None):
    """A*: la cola se ordena por f(n) = g(n)+h(n) (coste más heurística)."""

    return búsqueda_con_prioridad(problema,lambda n: n.coste_camino + h(n.estado))
